...
class Supplier extends Model implements JsonSerializable{
	...
	public function set($id,$name,$email,$phone,$address,$created_at,$updated_at){
		$this->id=$id;
		$this->name=$name;
		$this->email=$email;
		$this->phone=$phone;
	...
	public function save(){
		global $db,$tx;
		$db->query("insert into {$tx}suppliers(name,email,phone,address,created_at,updated_at)values('$this->name','$this->email','$this->phone','$this->address','$this->created_at','$this->updated_at')");
		return $db->insert_id;
	}
	public function update(){
		global $db,$tx;
		$db->query("update {$tx}suppliers set name='$this->name',email='$this->email',phone='$this->phone',address='$this->address',created_at='$this->created_at',updated_at='$this->updated_at' where id='$this->id'");
	}
	...
	public static function all(){
		global $db,$tx;
		$result=$db->query("select id,name,email,phone,address,created_at,updated_at from {$tx}suppliers");
		$data=[];
	...
	public static function pagination($page=1,$perpage=10,$criteria=""){
		global $db,$tx;
		$top=($page-1)*$perpage;
		$result=$db->query("select id,name,email,phone,address,created_at,updated_at from {$tx}suppliers $criteria limit $top,$perpage");
		$data=[];
		while($supplier=$result->fetch_object()){
			$data[]=$supplier;
		}
	...
	public static function find($id){
		global $db,$tx;
		$result =$db->query("select id,name,email,phone,address,created_at,updated_at from {$tx}suppliers where id='$id'");
...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		if($action){
			$html.="<tr><th>Id</th><th>Name</th><th>Email</th><th>Phone</th><th>Address</th><th>Created At</th><th>Updated At</th><th>Action</th></tr>";
		}else{
			$html.="<tr><th>Id</th><th>Name</th><th>Email</th><th>Phone</th><th>Address</th><th>Created At</th><th>Updated At</th></tr>";
		}
		while($supplier=$result->fetch_object()){
			$action_buttons = "";
			if($action){
			...
			$html.="<tr><td>$supplier->id</td><td>$supplier->name</td><td>$supplier->email</td><td>$supplier->phone</td><td>$supplier->address</td><td>$supplier->created_at</td><td>$supplier->updated_at</td> $action_buttons</tr>";
		}
		$html.="</table>";
