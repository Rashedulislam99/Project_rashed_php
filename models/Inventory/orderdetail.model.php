...
class OrderDetail extends Model implements JsonSerializable{
	...
	public function set($id,$order_id,$product_id,$qty,$unit_price,$vat,$discount,$created_at,$updated_at){
		$this->id=$id;
	...
	public function save(){
		global $db,$tx;
		$db->query("insert into {$tx}order_details(order_id,product_id,qty,unit_price,vat,discount,created_at,updated_at)values('$this->order_id','$this->product_id','$this->qty','$this->unit_price','$this->vat','$this->discount','$this->created_at','$this->updated_at')");
		return $db->insert_id;
	}
	public function update(){
		global $db,$tx;
		$db->query("update {$tx}order_details set order_id='$this->order_id',product_id='$this->product_id',qty='$this->qty',unit_price='$this->unit_price',vat='$this->vat ',discount='$this->discount',created_at='$this->created_at',updated_at='$this->updated_at' where id='$this->id'");
	}
	...
	public static function all(){
		global $db,$tx;
		$result=$db->query("select id,order_id,product_id,qty,unit_price,,vat,discount,created_at,updated_at from {$tx}order_details");
		$data=[];
	...
	public static function pagination($page=1,$perpage=10,$criteria=""){
		global $db,$tx;
		$top=($page-1)*$perpage;
		$result=$db->query("select id,order_id,product_id,qty,unit_price,vat,discount,created_at,updated_at from {$tx}order_details $criteria limit $top,$perpage");
		$data=[];
		while($orderdetail=$result->fetch_object()){
			$data[]=$orderdetail;
		}
	...
	public static function find($id){
		global $db,$tx;
		$result =$db->query("select id,order_id,product_id,qty,unit_price,vat,discount,created_at,updated_at from {$tx}order_details where id='$id'");
...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		if($action){
			$html.="<tr><th>Id</th><th>Order Id</th><th>Product Id</th><th>Qty</th><th>Unit Price</th><th>vat</th><th>Discount</th><th>Created At</th><th>Updated At</th><th>Action</th></tr>";
		}else{
			$html.="<tr><th>Id</th><th>Order Id</th><th>Product Id</th><th>Qty</th><th>Unit Price</th><th>vat</th><th>Discount</th><th>Created At</th><th>Updated At</th></tr>";
		}
		while($orderdetail=$result->fetch_object()){
			$action_buttons = "";
			if($action){
			...
			$html.="<tr><td>$orderdetail->id</td><td>$orderdetail->order_id</td><td>$orderdetail->product_id</td><td>$orderdetail->qty</td><td>$orderdetail->unit_price</td><td>$orderdetail->vat</td><td>$orderdetail->discount</td><td>$orderdetail->created_at</td><td>$orderdetail->updated_at</td> $action_buttons</tr>";
		}
		$html.="</table>";
    ...
    public static function find_all_by_order_id($order_id) {
        global $db, $tx;
        $table = "{$tx}order_details";
